...
if(!function_exists('ambil_nilai_aspek_kriteria'))
{
	function ambil_nilai_aspek_kriteria($kriteriaid,$dari,$tujuan)
	{
		$CI=& get_instance();
		$CI->load->library('m_db');
		$s=array(
		'id_kriteria'=>$kriteriaid,
		'aspek_kriteria_id_dari'=>$dari,
		'aspek_kriteria_id_tujuan'=>$tujuan,
		);
		$item=$CI->m_db->get_row('aspek_kriteria_nilai',$s,'nilai');
		return $item;
	}
}
